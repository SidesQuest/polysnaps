...
export function generateBuffZones(prestigeLevel, coreSides, skills) {
  const seed = (prestigeLevel + 1) * 7919 + coreSides * 31;
  const rng = seededRandom(seed);

  const zoneCount = 2 + prestigeLevel;
  const zones = [];
  const rangeBonus = skills ? skills._zoneRange || 0 : 0;
  const powerBonus = skills ? skills._zonePower || 0 : 0;

  for (let i = 0; i < zoneCount; i++) {
    const angle = rng() * Math.PI * 2;
    const distance = 120 + rng() * 200;
    const type = ZONE_TYPES[Math.floor(rng() * ZONE_TYPES.length)];
    const baseRadius = 50 + rng() * 30;

    zones.push({
      id: `zone-${prestigeLevel}-${i}`,
      x: Math.cos(angle) * distance,
      y: Math.sin(angle) * distance,
      radius: baseRadius * (1 + rangeBonus),
      ...type,
      multiplier: 1 + (type.multiplier - 1) * (1 + powerBonus),
    });
  }

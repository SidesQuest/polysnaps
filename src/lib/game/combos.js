...
const COMBO_DEFS = [
  ...
  // Secret combos (discoverable only)
  {
    id: "hex-trio",
    name: "Hexagonal Harmony",
    description: "Place 3 hexagons in one run",
    secret: true,
    check: (nodes) => nodes.filter((n) => n.shape === "hexagon").length >= 3,
    bonus: { type: "multiply", value: 1.35, target: "all" },
    icon: "⬡",
  },
  {
    id: "shape-rainbow",
    name: "Shape Rainbow",
    description: "Have all 4 shape types placed",
    secret: true,
    check: (nodes) => {
      const types = new Set(
        nodes.filter((n) => n.id !== "core").map((n) => n.shape),
      );
      return types.size >= 4;
    },
    bonus: { type: "multiply", value: 1.4, target: "all" },
    icon: "🌈",
  },
  {
    id: "deep-abyss",
    name: "Into the Void",
    description: "Build 7 layers deep",
    secret: true,
    check: (nodes) => {
      function getDepth(nodeId) {
        let d = 0,
      ...
      return nodes.some((n) => getDepth(n.id) >= 7);
    },
    bonus: { type: "multiply", value: 1.75, target: "all" },
    icon: "🌌",
  },
  {
    id: "fifty-shapes",
    name: "Geometric Singularity",
    description: "Place 50 shapes",
    secret: true,
    check: (nodes) => nodes.length - 1 >= 50,
    bonus: { type: "multiply", value: 2.0, target: "all" },
    icon: "💫",
  },
];
...
export function getDiscoveredCombos(discoveredIds = []) {
  return COMBO_DEFS.filter((c) => discoveredIds.includes(c.id));
}

export function getTotalComboCount() {
  return COMBO_DEFS.length;
}
